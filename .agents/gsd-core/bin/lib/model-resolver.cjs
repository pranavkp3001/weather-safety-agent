 ...
 * or a non-the agent vendor id (`openai/o3`) — is emitted verbatim as pinned.
...
function warnModelOverrideUnmappable(agentType, overrideValue) {
    ...
    process.stderr.write(`gsd- warning — model_overrides value "${safe}" for ${agentType} is a fully-qualified ` +
        `the agent model ID with no tier alias; passing it through verbatim. Claude Code setups ` +
 ...
 * Non-the agent runtimes and non-the agent values always pass through verbatim.
 *
