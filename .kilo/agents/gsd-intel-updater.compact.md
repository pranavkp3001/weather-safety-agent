...
**Project skills:** check `.kilo/skills/` or `.kilo/skills/` if either exists — list skill subdirectories; read each `SKILL.md` (~130 lines); load `rules/*.md` as needed; do NOT load full `AGENTS.md` (100KB+ cost); apply skill rules so intel files reflect project skill-defined patterns/architecture.
